                                           ...
                                           const InterceptTable * table ) const
{
    ...
    for ( std::size_t i = 0; i < MAX; ++i )
    {
       ...
       const Vector2D ball_vel
        ...
        const int opp_buf = ( attacker ? 1 : 4 );
                                ...
                                const InterceptInfo & info )
{
    ...
    {
        if ( inertia_dist < wm.self().kickableArea() - 0.15 - ball_noise
             ...
             )
        {
            double next_kick_rate
                = wm.self().playerType().kickRate( ServerParam::i(),
                                                   inertia_dist,
                                                   inertia_rel.th().abs() );
    ...
    if ( faced_rel.absY() > wm.self().kickableArea() - ball_noise - 0.2 )
    {
        dlog.addText( Logger::INTERCEPT,
    ...
    double dist_buf = ( wm.self().kickableArea()
                        - 0.3
                        + extra_buf );
