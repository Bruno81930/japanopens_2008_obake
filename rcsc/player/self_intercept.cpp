                              ...
                              std::vector< InterceptInfo > & self_cache ) const
{
    ...
    double ball_noise = M_world.ball().vel().r() * ServerParam::i().ballRand() * 1.4;
                                        ...
                                        const double & max_forward_accel_x ) const
{
    ...
    required_accel_x[1]
        = next_ball_rel.x
        + std::sqrt( std::pow( best_ctrl_dist - 0.1, 2.0 )
                     - std::pow( next_ball_rel.y, 2.0 ) );
                                ...
                                std::vector< InterceptInfo > & self_cache ) const
{
    ...
    for ( int cycle = min_cycle; cycle < max_loop; ++cycle )
    {
        ...
        if ( ball_pos.absX() > ServerParam::i().pitchHalfLength() + 10.0
             || ball_pos.absY() > ServerParam::i().pitchHalfWidth() + 10.0
             )
        {
#ifdef DEBUG
            dlog.addText( Logger::INTERCEPT,
    ...
    {
        dlog.addText( Logger::INTERCEPT,
                      "%s: SelfIntercept. failed to predict? register ball final point",
                      __FILE__ );
