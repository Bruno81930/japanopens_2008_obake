...
Body_Dribble2006::execute( PlayerAgent * agent )
{
    dlog.addText( Logger::TEAM,
                  "%s:%d: Body_Dribble. (%.1f, %.1f), dash_power= %.1f"
                  " dash_count=%d"
                  ,__FILE__, __LINE__,
    ...
    dlog.addText( Logger::TEAM,
                  "%s:%d: dash_power=%.1f, dash_count=%d"
                  ,__FILE__, __LINE__,
                  M_dash_power, M_dash_count );
                            ...
                            const bool dodge )
{
    ...
    {
        dlog.addText( Logger::TEAM,
                      "%s:%d: need turn to (%.2f, %.2f) angle=%.1f, diff=%.1f"
                      ,__FILE__, __LINE__,
        ...
        {
            dlog.addText( Logger::TEAM,
                          "%s:%d: next kickable. next_dist=%.2f"
                          ,__FILE__, __LINE__,
                          ball_next_dist );
                 ...
                 )
            {
                ...
                dlog.addText( Logger::TEAM,
                              "%s:%d: next kickable. next_dist=%.2f,  turn=%.1f"
                              ,__FILE__, __LINE__,
                              ...
                              const double & dash_power )
{
    ...
    {
        dlog.addText( Logger::TEAM,
                      "%s:%d: doTurnOnly  exist near opponent"
                      ,__FILE__, __LINE__ );
        return false;
    }
    ...
    {
        dlog.addText( Logger::TEAM,
                      "%s:%d: doTurnOnly  not kickable at next"
                      ,__FILE__, __LINE__ );
        return false;
    }
    ...
    dlog.addText( Logger::TEAM,
                  "%s:%d: kickable at next and required only one turn."
                  " next_ball_dist=%.2f"
                  ,__FILE__, __LINE__,
                  ball_next_dist );
                                   ...
                                   const double & dash_power )
{
    ...
    if ( doCollideForTurn( agent, dir_diff_abs, true ) )
    {
        ...
        dlog.addText( Logger::DRIBBLE,
                      "%s:%d: KickTurnsDash. Collide with ball"
                      ,__FILE__, __LINE__ );
        return true;
    }
    ...
    // plus dash move
#if 0
    my_pos += Vector2D::polar2vector
        ( std::fabs( dash_power ) * wm.self().dashRate(),
          target_angle );
#endif
    double control_dist
        = wm.self().kickableArea() * 0.7;
    ...
    {
        dlog.addText( Logger::DRIBBLE,
                      "%s:%d: KickTurnsDash. power over= %f,  or speed range over= %f"
                      ,__FILE__, __LINE__,
    ...
    // check collision
    {
        ...
        for ( int i = 1; i < n_turn + 1; i++ )
        {
            ...
            {
                dlog.addText( Logger::DRIBBLE,
                              "%s:%d: KickTurnsDash. maybe cause collision"
                              ,__FILE__, __LINE__ );
                                ...
                                const int dash_count )
{
    ...
    for ( int i = 0; i < dash_count; i++ )
    {
...
#ifdef DEBUG
            dlog.addText( Logger::DRIBBLE,
                          "%s:%d: doKickDashes. estimate speed over. dash %d. power conserve"
        ...
        dlog.addText( Logger::DRIBBLE,
                      "%s:%d: doKickDashes. predict my move. dash %d."
    ...
    dlog.addText( Logger::DRIBBLE,
                  "%s:%d: doKickDashes. my move dist = %.3f  dir = %.1f  accel_angle=%.1f"
                  ,__FILE__, __LINE__,
    ...
    if ( exist_close_opp )
    {
        ...
        if ( my_move_dir.isLeftOf( keep_global_angle ) )
        {
            ...
            dlog.addText( Logger::DRIBBLE,
                          "%s:%d: KickDashes. avoid. keep right"
                          ,__FILE__, __LINE__ );
        }
        else
        {
            ...
            keep_global_angle = my_move_dir - add_angle_abs;
            dlog.addText( Logger::DRIBBLE,
                          "%s:%d: KickDashes. avoid. keep left"
                          ,__FILE__, __LINE__ );
        }
    }
    else
    ...
    dlog.addText( Logger::DRIBBLE,
                  "%s:%d: KickDashes. next_ball_rel=(%.2f, %.2f) global(%.2f %.2f)"
    ...
    {
        dlog.addText( Logger::DRIBBLE,
                      "%s:%d: KickDashes. never reach. rotate."
                      ,__FILE__, __LINE__ );
         ...
         < collide_dist2 )
    {
        ...
        dlog.addText( Logger::DRIBBLE,
                      "%s:%d: KickDashes. maybe collision. rotate. rel_angle=%f"
                      ,__FILE__, __LINE__,
    ...
    agent->setIntention
        ( new IntentionDribble2006( target_point,
                                    ...
                                    wm.time() ) );
    dlog.addText( Logger::DRIBBLE,
                  "%s:%d: KickDashes. register intention. dash_count=%d"
                  ,__FILE__, __LINE__,
                  dash_count );
    // execute first kick
    return agent->doKick( required_kick_power,
                                        ...
                                        const double & dash_power )
{
    ...
    const double kickable_area
        = wm.self().kickableArea() - 0.25;
    const Rect2D pitch_rect( - ServerParam::i().pitchHalfLength(),
    ...
    const double max_dist = wm.self().kickableArea() - 0.1;
    ...
    while ( first_ball_dist < max_dist )
    {
              ...
              ++angle_loop, first_ball_angle += 180.0 / ANGLE_DIVS )
        {
            ...
            if ( first_ball_vel.r() / wm.self().kickRate()
    ...
    dlog.addText( Logger::DRIBBLE,
                  "%s:%d: KickDashesWithKeep.dash_count=%d, vel=(%.1f %.1f)"
                  ,__FILE__, __LINE__,
                  dribble.dash_count_,
                                    ...
                                    double * dash_power )
{
    ...
    if ( ( ( ball_pos - my_pos ).th() - accel_angle ).abs() < 150.0
         && ball_dist + noise_buf < wm.self().kickableArea() - 0.1
         && ( ball_dist - noise_buf
              > ( wm.self().playerType().playerSize()
                  + ServerParam::i().ballSize() ) ) )
    {
...
{
    dlog.addText( Logger::DRIBBLE,
                  "%s:%d: existCloseOppnent. check start."
                  ,__FILE__, __LINE__ );
    ...
    {
        dlog.addText( Logger::DRIBBLE,
                      "%s:%d: existCloseOppnent. No opponent."
                      ,__FILE__, __LINE__ );
        return false;
    }
         ...
         )
    {
        ...
        *keep_angle = (opp_angle - 180.0);
        ...
        dlog.addText( Logger::DRIBBLE,
                      "%s:%d: existCloseOpponent  found interfere opponent."
                      " keep_angle=%.1f"
                      ,__FILE__, __LINE__,
                      keep_angle->degree() );
        return true;
    ...
    dlog.addText( Logger::DRIBBLE,
                  "%s:%d:existCloseOpponent. No dangarous opponent"
                  ,__FILE__, __LINE__ );
    return false;
}
                                 ...
                                 const AngleDeg & target_angle )
{
    ...
    {
        dlog.addText( Logger::DRIBBLE,
                      "%s: getAvoidAngle. check body lien."
                      ,__FILE__ );
