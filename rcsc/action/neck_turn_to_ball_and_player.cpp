...
Neck_TurnToBallAndPlayer::execute( PlayerAgent * agent )
{
    ...
    dlog.addText( Logger::ACTION,
                  "%s: ball_angle = %.1f  player_angle = %.1f  my_next_body = %.1f"
                  ,__FILE__,
    ...
    {
        dlog.addText( Logger::ACTION,
                      "%s:%d: cannot face to ball and player %d (%.1f %.1f)"
                      ,__FILE__, __LINE__,
                      M_target_player->unum(),
    ...
    {
        dlog.addText( Logger::ACTION,
                      "%s:%d: cannot face to player %d (%.1f %.1f)"
                      ,__FILE__, __LINE__,
                      M_target_player->unum(),
                      player_next.x, player_next.y );
    ...
    {
        dlog.addText( Logger::ACTION,
                      "%s:%d: cannot look both ball and player %d (%.1f %.1f)"
                      ,__FILE__, __LINE__,
                      M_target_player->unum(),
                      player_next.x, player_next.y );
    ...
    dlog.addText( Logger::ACTION,
                  "%s:%d: face to ball and player %d (%.1f %.1f), face_dir = %.1f "
                  ,__FILE__, __LINE__,
                  M_target_player->unum(),
                  player_next.x, player_next.y,
                  target_angle.degree() );
