...
ServerParam::setAdditionalParam()
{
    ...
    M_catchable_area = std::sqrt( std::pow( catchAreaWidth() * 0.5, 2.0 )
                                  + std::pow( catchAreaLength(), 2.0 ) );
    M_control_radius_width = M_control_radius - M_player_size;

    // real speed max
    double accel_max = maxPower() * defaultDashPowerRate() * defaultEffortMax();
...
ServerParam::convertFrom( const rcg::server_params_t & from )
{
    ...
    if ( 0.0 < tmp && std::fabs( tmp ) < 1000.0 ) M_max_tackle_power = tmp;
    tmp = rcg::nltohd( from.max_back_tackle_power );
    if ( 0.0 < tmp && std::fabs( tmp ) < 1000.0 ) M_max_back_tackle_power = tmp;

    tmp = rcg::nltohd( from.tackle_dist );
    if ( 0.0 <= tmp && std::fabs( tmp ) < 1000.0 ) M_tackle_dist = tmp;
    tmp = rcg::nltohd( from.tackle_back_dist );
    if ( 0.0 <= tmp && std::fabs( tmp ) < 1000.0 ) M_tackle_back_dist = tmp;
    tmp = rcg::nltohd( from.tackle_width );
    if ( 0.0 < tmp && std::fabs( tmp ) < 1000.0 ) M_tackle_width = tmp;
