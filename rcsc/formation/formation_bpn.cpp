...
{
    double msign = 1.0;
    if ( type == Formation::SYNMETRY ) msign =  -1.0;
                             ...
                             const Formation::SideType type )
{
    ...
    {
        std::cerr << __FILE__ << ":" << __LINE__
                  << " *** ERROR *** You cannot create a new parameter for synmetry type."
                  << std::endl;
        return;
    }
...
FormationBPN::train( const std::list< Snapshot > & train_data )
{
    ...
    for ( int unum = 1; unum <= 11; ++unum )
    {
        ...
        {
            std::cerr << "  Training. player " << unum << " >>> synmetry type "
                      << std::endl;
            continue;
        }
...
FormationBPN::param( const int unum ) const
{
    ...
    const int player_number = ( M_synmetry_number[unum - 1] <= 0 // center or original
                                ? unum
