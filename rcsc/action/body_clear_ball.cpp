...
const double Body_ClearBall::S_SEARCH_ANGLE = 8.0;

GameTime Body_ClearBall::S_last_calc_time( 0, 0 );
AngleDeg Body_ClearBall::S_cached_best_angle = 0.0;
...
*/
bool
Body_ClearBall::execute( PlayerAgent * agent )
{
    ...
    else
    {
        ...
        agent->setIntention
            ( new IntentionKick( target_point,
                                 ...
                                 agent->world().time() ) );
        dlog.addText( Logger::TEAM,
                      "%s:%d: register clear kick intention"
                      ,__FILE__, __LINE__ );
    }
    return true;
}
...
*/
AngleDeg
Body_ClearBall::get_best_angle( const PlayerAgent * agent,
                                ...
                                const bool clear_mode )
{
    ...
    do
    {
        ...
        tmp_score *= std::pow( 0.95, static_cast< double >( dir_count ) );
        ...
        dlog.addText( Logger::CLEAR,
                      "Body_ClearBall.get_best_angle. search_angle=%f, score=%f",
...
*/
double
Body_ClearBall::calc_score( const PlayerAgent * agent,
